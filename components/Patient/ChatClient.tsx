...
export default function ChatPage({ medecinId }: ChatPageProps) {
  ...
  useEffect(() => {
    ...
    socket.emit("join-room", medecinId);
    ...
    // Cleanup : déconnexion et suppression des listeners
    return () => {
      socket.off("receive-message", handleReceiveMessage); // retire l'event listener
      socket.disconnect(); // déconnexion propre
      socketRef.current = null; // réinitialise la ref
    };
  }, [medecinId]);
