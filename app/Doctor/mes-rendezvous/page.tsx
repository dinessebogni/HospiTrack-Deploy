...
export default function RendezVousPage() {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:8000/api/rendez-vous?medecinId=${medecinId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!res.ok) throw new Error('Erreur récupération rendez-vous');
        const data: RendezVous[] = await res.json();
  ...
  const handleAction = async (rv: RendezVous, action: 'confirm' | 'cancel') => {
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/rendez-vous/${rv._id}/${action}`, {
        ...
        body: JSON.stringify({ status: action === 'confirm' ? 'confirme' : 'annule' }),
      });
        ...
        {appointments.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white dark:bg-gray-800 rounded-xl shadow-md">
              ...
              <tbody>
                {appointments.map(rv => (
                  <tr key={rv._id} className="border-b dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
